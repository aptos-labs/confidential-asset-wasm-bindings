...
export function hexToBytes(fieldName: string, value: string, expectedByteLength?: number): Uint8Array {
  ...
  if (trimmed.length % 2 !== 0) {
    throw new Error(`${fieldName} must contain an even number of hex characters`)
  }
...
export function parsePositiveInteger(fieldName: string, value: string, options?: { max?: number }): number {
...
export function parseNonNegativeInteger(fieldName: string, value: string): number {
  const trimmed = value.trim()
...
export function parseCsvIntegers(fieldName: string, value: string, options?: { min?: number; max?: number }): number[] {
  const items = value
    .split(',')
    .map((item) => item.trim())
    .filter(Boolean)
