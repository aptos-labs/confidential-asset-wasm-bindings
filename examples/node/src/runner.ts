...
function splitFixedWidth(flat: Uint8Array, count: number): Uint8Array[] {
  return Array.from({ length: count }, (_, i) => new Uint8Array(flat.subarray(i * ELEMENT_WIDTH, (i + 1) * ELEMENT_WIDTH)))
}
...
function cloneRuntimeState(runtimeState?: ManualRuntimeState): ManualRuntimeState {
  return {
...
export function runtimeStateToDerivedOutputs(runtimeState: ManualRuntimeState): ManualDerivedOutputs {
  return {
    batchProofHex: runtimeState.batchProof ? bytesToHex(runtimeState.batchProof) : '',
    batchCommsFlatHex: runtimeState.batchCommsFlat ? bytesToHex(runtimeState.batchCommsFlat) : '',
    batchCount: runtimeState.batchCount === null ? '' : String(runtimeState.batchCount),
  }
}

function pushSkippedResult(results: ManualStepResult[], key: ManualStepKey, reason: string) {
  results.push({
    key,
...
export function prepareManualTestInput(form: ManualTestFormState): PrepareResult {
  ...
  const solverPoint = readField('solverPointHex', () => hexToBytes('point', form.solverPointHex, 32))
  const solverMaxNumBits = readField('solverMaxNumBits', () =>
    parsePositiveInteger('maxNumBits', form.solverMaxNumBits, { max: 255 })
  )
  const expectedSolveValue = readField('expectedSolveValue', () => parseU64('Expected solve value', form.expectedSolveValue))
  const valBase = readField('valBaseHex', () => hexToBytes('valBase', form.valBaseHex, 32))
  const randBase = readField('randBaseHex', () => hexToBytes('randBase', form.randBaseHex, 32))
  const batchValues = readField('batchValuesCsv', () => parseCsvU64('values', form.batchValuesCsv))
  const batchBlindingsFlat = batchValues
    ? readField('batchBlindingsFlatHex', () =>
        hexToBytes('blindingsFlat', form.batchBlindingsFlatHex, batchValues.length * 32)
      )
    : null
  ...
  const expectedBatchProofBytes = readField('expectedBatchProofBytes', () =>
    parsePositiveInteger('Expected batch proof bytes', form.expectedBatchProofBytes)
  )
  const expectedBatchCount = readField('expectedBatchCount', () =>
    parsePositiveInteger('Expected batch commitment count', form.expectedBatchCount)
  )
  const expectedBatchCommsFlat = expectedBatchCount
    ? readField('expectedBatchCommsFlatHex', () =>
        bytesToHex(hexToBytes('Expected batch commitments', form.expectedBatchCommsFlatHex, expectedBatchCount * 32))
      )
    : null
  ...
  for (const key of MANUAL_STEP_KEYS) {
    const budgetValue = readField(budgetFieldId(key), () =>
      parseNonNegativeInteger(`${MANUAL_STEP_LABELS[key]} budget`, form.budgets[key])
    )
...
async function measure<T>(now: () => number, action: () => Promise<T>): Promise<{ durationMs: number; value?: T; error?: unknown }> {
  const startedAt = now()
  ...
  switch (key) {
    case 'solveDiscreteLog': {
      const stepResult = await measure(now, () =>
        solveDiscreteLog(input.solver.point, input.solver.maxNumBits).then((n) => n.toString())
      )
      if (stepResult.error) {
        pushMeasuredResult(results, key, stepResult.durationMs, 'No discrete log value returned', errorMessage(stepResult.error), false, input.budgets[key])
        return
      }
    ...
    case 'batchRangeProof': {
      const rs = splitFixedWidth(input.batch.blindingsFlat, input.batch.valueCount)
      const stepResult = await measure(now, () =>
        batchRangeProof({
      ...
      if (stepResult.error) {
        pushMeasuredResult(results, key, stepResult.durationMs, 'No batch proof returned', errorMessage(stepResult.error), false, input.budgets[key])
        return
      }

      const { proof, comms } = stepResult.value!
      const commsFlat = new Uint8Array(comms.length * ELEMENT_WIDTH)
      comms.forEach((c, i) => commsFlat.set(c, i * ELEMENT_WIDTH))
    ...
    case 'batchVerifyProof': {
      if (!runtimeState.batchProof || !runtimeState.batchCommsFlat || runtimeState.batchCount === null) {
        pushSkippedResult(results, key, 'batchVerifyProof requires batchRangeProof() output')
        return
      }

      const comms = splitFixedWidth(runtimeState.batchCommsFlat, runtimeState.batchCount)
      const stepResult = await measure(now, () =>
        batchVerifyProof({
      ...
      if (stepResult.error) {
        pushMeasuredResult(results, key, stepResult.durationMs, 'No batch verification result returned', errorMessage(stepResult.error), false, input.budgets[key])
        return
      }
...
export async function runManualTestSuite(
  ...
  return executeSteps(input, MANUAL_STEP_KEYS, createEmptyRuntimeState(), options)
}
