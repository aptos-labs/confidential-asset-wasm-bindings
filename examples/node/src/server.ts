...
import { prepareManualTestInput, runManualTestStep, runManualTestSuite, runtimeStateToDerivedOutputs } from './runner.js'
import { createEmptyRuntimeState, MANUAL_STEP_KEYS, type ManualStepKey, type ManualTestFormState } from './types.js'
...
async function handleRunAll(res: http.ServerResponse, overrides: unknown): Promise<void> {
...
async function handleRunStep(res: http.ServerResponse, step: ManualStepKey, overrides: unknown): Promise<void> {
  const form = mergeForm(overrides)
  const { errors, prepared } = prepareManualTestInput(form)
  ...
  const result = await runManualTestStep(prepared, step, createEmptyRuntimeState())
  send(res, 200, result)
}
  ...
  try {
    ...
    if (method === 'POST' && path.startsWith('/run/')) {
      ...
      if (!(MANUAL_STEP_KEYS as readonly string[]).includes(stepKey)) {
        send(res, 404, { error: `Unknown step "${stepKey}". Valid steps: ${MANUAL_STEP_KEYS.join(', ')}` })
        return
      }
      const body = await readBody(req)
...
server.listen(PORT, () => {
  ...
  console.log('  POST /run/:step      run one step (steps: ' + MANUAL_STEP_KEYS.join(', ') + ')')
})
